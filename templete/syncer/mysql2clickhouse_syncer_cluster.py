...
def get_ck_table_defi(cfg,event):
    ...
    st= 'create table `{}`.`{}` on cluster {} (\n '.format(get_ck_schema(cfg,event),event['table'],cfg['ch_cluster_name'])
    for i in rs:
        if i[1] == 'tinyint':
           ...
           st = st + ' `{}` {},\n'.format(i[0],'DateTime' if not is_col_null(i[2])  else 'Nullable(DateTime)' )
            ...
            st = st + ' `{}` {},\n'.format(i[0],'String'  if not is_col_null(i[2]) else 'Nullable(String)' )
    st = st[0:-2]+') \n' + cfg['ck_config'].format(event['table'])
    return st

def get_ck_table_defi_all(cfg,event):
    st = """SELECT  `column_name`,data_type,is_nullable,datetime_precision,column_default
              FROM information_schema.columns
              WHERE table_schema='{}'
                AND table_name='{}'  ORDER BY ordinal_position""".format(event['schema'],event['table'])
    cfg['cr_mysql'].execute(st)
    rs = cfg['cr_mysql'].fetchall()
    st= 'create table `{}`.`{}` on cluster {} (\n '.format(get_ck_schema(cfg,event)+'_all',event['table'],cfg['ch_cluster_name'])
    for i in rs:
        if i[1] == 'tinyint':
            st = st + ' `{}` {},\n'.format(i[0],'Int16'   if not is_col_null(i[2])  else 'Nullable(Int16)')
        elif i[1] == 'int':
...
def get_tab_header(cfg,event):
    s1 = "insert into `{}`.`{}` (".format(get_mysql_schema(cfg,event)+'_all',event['table'].lower())
    s2 = " values "
...
def create_ck_table(cfg,event):
    ...
    if check_tab_exists_pk(cfg,event) >0:
        if check_ck_database(cfg,event) == 0:
           db.execute('create database if not exists {} on cluster {}'.format(get_ck_schema(cfg, event),cfg['ch_cluster_name']))
           logging.info('\033[0;36;40m clickhouse => create  database if not exists `{}` on cluster {} success!\033[0m'.format(get_ck_schema(cfg, event),cfg['ch_cluster_name']))
           db.execute('create database if not exists {} on cluster {}'.format(get_ck_schema(cfg, event)+'_all',cfg['ch_cluster_name']))
           logging.info('\033[0;36;40m clickhouse => create  database if not exists `{}` on cluster {} success!\033[0m'.format(get_ck_schema(cfg, event)+'_all', cfg['ch_cluster_name']))
        ...
        if event['column'] =='auto' or event.get('column') is None:
           try:
              ...
              logging.info('\033[0;36;40m clickhouse => create  table1 `{}.{}` success!\033[0m'.format(get_ck_schema(cfg, event),event['table']))
           except:
              traceback.print_exc()
        else:
           ...
           try:
              ...
              logging.info('\033[0;36;40m clickhouse => create  table2 `{}.{}` success!\033[0m'.format(get_ck_schema(cfg, event),event['table']))
           except:
              traceback.print_exc()

        # create distributed table
        st = get_ck_table_defi_all(cfg,event)
        db.execute(st)
        logging.info('\033[0;36;40m clickhouse => create distributed table `{}.{}` success!\033[0m'.format(get_ck_schema(cfg, event)+'_all',event['table']))
...
def get_ck_table_defi_mysql(cfg,event):
    ...
    cr.execute(st)
    rs = cr.fetchall()
    st= 'create table `{}`.`{}` (\n '.format(get_ck_schema(cfg,event)+'_all',event['table']+'_tmp')
    for i in rs:
        if i[1] == 'tinyint':
...
def get_ins_header(cfg,event):
    v_ddl = 'insert into {0}.{1} ('.format(get_ck_schema(cfg,event)+'_all', event['table'])
...
def get_config_from_db(tag):
    ...
    if res['code'] == 200:
        ...
        config['db_ck_ip']               = db_ck_ip.split(',')[0]
        ...
        config['db_ck']                  = get_ds_ck(config['db_ck_ip'], db_ck_port, db_ck_service, db_ck_user, db_ck_pass)
        ...
        config['ck_config_all'] = CK_TAB_CONFIG_ALL
