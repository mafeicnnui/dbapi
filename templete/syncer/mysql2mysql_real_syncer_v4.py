...
def check_tab_exists_pk_thread(cfg,event):
   db = get_ds_mysql_sour(
                cfg['db_mysql_ip'],
                cfg['db_mysql_port'],
...
def get_sync_table_cols_new(cr,event):
    st="""select concat('`',column_name,'`') from information_schema.columns
              where table_schema='{0}' and table_name='{1}'  order by ordinal_position
          """.format(event['schema'],event['table'])
    cr.execute(st)
...
def get_tab_header_new(cfg,cr,event):
    s1 = "insert into `{}`.`{}` (".format(get_mysql_schema(cfg,event),event['table'].lower())
    s2 = " values "
    s1 = s1+get_sync_table_cols_new(cr,event)+")"
    s3 = ','.join(['%s' for i in get_sync_table_cols_new(cr,event).split(',')])
    return s1+s2+'('+s3+')'
...
def get_sync_table_total_rows_thread(f_cfg,event):
    db = get_ds_mysql_sour(
        f_cfg['db_mysql_ip'],
        f_cfg['db_mysql_port'],
        f_cfg['db_mysql_service'],
        f_cfg['db_mysql_user'],
        f_cfg['db_mysql_pass'])
    cr = db.cursor()
    st = "select count(0) from `{0}`.`{1}`".format(event['schema'],event['table'])
...
def init_tasks(f_cfg,event,n_total_rows,stdscr):
    f_cfg['counter'] = {}
    thread_avg_rows = int(n_total_rows / f_cfg['process_num'])
    ...
    db = get_ds_mysql_sour(
                ...
                f_cfg['db_mysql_pass'])
    cr = db.cursor()
    cr.execute('select min(id),max(id) from `{}`.`{}`'.format(event['schema'],event['table']))
    tab_min_id,tab_max_id = cr.fetchone()
    thread_min_id=tab_min_id
    thread_max_id=thread_min_id+thread_avg_rows
    cr.execute('select count(0) from `{}`.`{}` where id between {} and {}'.format(event['schema'], event['table'],thread_min_id,thread_max_id))
    thread_rows = cr.fetchone()[0]
    task = []
    thread_id = 1
    while thread_max_id < tab_max_id:
        if thread_rows>0:
            task.append((f_cfg, event, n_total_rows,
                         thread_id, thread_rows,
                         thread_min_id,thread_max_id,start_time,stdscr))
        f_cfg['counter']['thread_' + str(thread_id)] = {}
        thread_min_id = thread_max_id+1
        thread_max_id = thread_max_id+thread_avg_rows
        if thread_max_id>tab_max_id:
           thread_max_id = tab_max_id
        cr.execute('select count(0) from `{}`.`{}` where id between {} and {}'
                   .format(event['schema'], event['table'],thread_min_id, thread_max_id))
        thread_rows = cr.fetchone()[0]
        thread_id += 1
    #last
    if thread_rows:
...
def init_tasks_v2(f_cfg,event,n_total_rows,stdscr):
    f_cfg['counter'] = {}
    avg_rows = int(n_total_rows / f_cfg['process_num'])
    start_time = int(time.time())
    db = get_ds_mysql_sour(
                f_cfg['db_mysql_ip'],
                f_cfg['db_mysql_port'],
    ...
    cr = db.cursor()
    cr.execute('select min(id),max(id) from `{}`.`{}`'.format(event['schema'],event['table']))
    thd_min_id,tab_max_id = cr.fetchone()
    #print('thd_min_id,tab_max_id=',thd_min_id,tab_max_id)
    cr.execute('select id from `{}`.`{}` where id>{} limit {},1'.format(event['schema'], event['table'], thd_min_id, avg_rows))
    thd_max_id = cr.fetchone()[0]
    cr.execute('select count(0) from `{}`.`{}` where id between {} and {}'.format(event['schema'], event['table'], thd_min_id,thd_max_id))
    thd_rows = cr.fetchone()[0]
    task = []
    thread_id = 1
    while thd_rows>0:
       f_cfg['counter']['thread_' + str(thread_id)] = {}
       task.append((f_cfg, event, n_total_rows,
                 thread_id, thd_rows,
                 thd_min_id, thd_max_id, start_time, stdscr))
       #print(thread_id, thd_rows,thd_min_id, thd_max_id)
       cr.execute('select MIN(id),COUNT(0) from `{}`.`{}` where id>{} and id<={}'.format(event['schema'], event['table'],thd_max_id,tab_max_id))
       thread_id += 1
       thd_min_id, remaining_rows = cr.fetchone()
       if remaining_rows >= avg_rows:
          cr.execute('select id from `{}`.`{}` where id>{} limit {},1'.format(event['schema'], event['table'], thd_max_id,avg_rows))
          thd_max_id = cr.fetchone()[0]
          cr.execute('select count(0) from `{}`.`{}` where id between {} and {}'.format(event['schema'], event['table'],thd_min_id, thd_max_id))
          thd_rows = cr.fetchone()[0]
       else:
          thd_max_id = tab_max_id
          cr.execute('select count(0) from `{}`.`{}` where id > {} and id<={}'.format(event['schema'], event['table'],thd_min_id,thd_max_id))
          thd_rows = cr.fetchone()[0]
          f_cfg['counter']['thread_' + str(thread_id)] = {}
          task.append((f_cfg, event, n_total_rows,
                       thread_id, thd_rows,
                       thd_min_id, thd_max_id, start_time, stdscr))
          #print('last : ',thread_id, thd_rows, thd_min_id, thd_max_id)
          break
    # for t in task:
    #     print(t[3],t[4],t[5],t[6])
    return task

def split_data(p_min_id,p_max_id,p_batch):
    r = []
    min_id = p_min_id
    max_id = min_id + p_batch-1
    while max_id!=p_max_id:
        r.append({
            'start_id':min_id,
            'stop_id':max_id
        })
...
def full_sync_thread_task(f_cfg,event,n_total_rows,n_thread_id,n_thread_rows, n_thread_min_id,n_thread_max_id,n_start_time,stdscr):
    db_mysql = get_ds_mysql_sour(
                            f_cfg['db_mysql_ip'],
                            f_cfg['db_mysql_port'],
                            f_cfg['db_mysql_service'],
                            f_cfg['db_mysql_user'],
                            f_cfg['db_mysql_pass'])
    db_mysql_dest = get_ds_mysql_dest(
                            f_cfg['db_mysql_ip_dest'],
                            f_cfg['db_mysql_port_dest'],
                            f_cfg['db_mysql_service_dest'],
                            f_cfg['db_mysql_user_dest'],
                            f_cfg['db_mysql_pass_dest'])
    cr_source   = db_mysql.cursor()
    cr_desc     = db_mysql_dest.cursor()
    ins_header  = get_tab_header_new(f_cfg, cr_source,event)
    n_counter   = 0
    for p in split_data(n_thread_min_id,n_thread_max_id,f_cfg['batch_size_incr']):
        line = 0
        st = "select * from `{}`.`{}` where id between {} and {}".format(event['schema'], event['table'],p['start_id'], p['stop_id'])
        cr_source.execute(st)
        rs_source = cr_source.fetchall()
        cr_desc.executemany(ins_header, rs_source)
        n_counter += len(rs_source)
        f_cfg['counter']['thread_'+str(n_thread_id)].update({
            'thread_id'      : n_thread_id,
            'thread_rows'    : n_thread_rows,
            'counter'        : n_counter,
            'min_id'         : n_thread_min_id,
            'max_id'         : n_thread_max_id,
            'total_rows'     : n_total_rows,
            'processed_rows' : sum([v.get('counter',0) for v in f_cfg['counter'].values()]),
            'status'         : 'RUNNING'
        })
        results = 'Thread:{} Table:`{}.{}` Rows:{}/{} Batch:{} Process:{}% Elaspse:{}s.'.\
                  format(n_thread_id,
                         event['schema'],event['table'],
                         n_total_rows,
                         sum([v.get('counter', 0) for v in f_cfg['counter'].values()]),
                         str(f_cfg['batch_size_incr']),
                         round(sum([v.get('counter', 0) for v in f_cfg['counter'].values()])/n_total_rows*100,2),
                         int(time.time()-n_start_time),
                         '-'*110)
        stdscr.addstr(line, 0, '-'*110,curses.color_pair(1))
        stdscr.addstr(line+1, 0, '{}'.format(results),curses.color_pair(1))
        stdscr.addstr(line+2, 0, '-'*110,curses.color_pair(1))
        results=''
        results_complete = ''
        running_rows  = 0
        for k,v in  f_cfg['counter'].items():
            if v != {} and v.get('status') == 'RUNNING':
                result = "{}: Id :{} Rows:{} Process:{}%". \
                    format(str(k).capitalize().ljust(10,' '),
                           '[{}~{}]'.format(str(v['min_id']),str(v['max_id'])).ljust(22,' '),
                           '{}/{}'.format(v['thread_rows'],v['counter']).ljust(20,' '),
                           round(v['counter'] / v['thread_rows'] * 100, 2))
                results=results+result+'\n'
                running_rows+=1
            elif v != {} and v.get('status') == 'COMPLETE':
                result_ok= "{}: Id :{} Rows:{} Process:{}%". \
                    format(str(k).capitalize().ljust(10, ' '),
                           '[{}~{}]'.format(str(v['min_id']), str(v['max_id'])).ljust(22, ' '),
                           '{}/{}'.format(v['thread_rows'], v['counter']).ljust(20, ' '),
                           round(v['counter'] / v['thread_rows'] * 100, 2))
                results_complete = results_complete + result_ok + '\n'
            else:
                pass
        stdscr.addstr(line+3,0,'{}'.format(results))
        if results_complete!='':
            stdscr.addstr(line+3+running_rows+2,0,'Complete Threads:', curses.color_pair(1))
            stdscr.addstr(line+3+running_rows+3, 0, '-' * 110, curses.color_pair(1))
            stdscr.addstr(line+3+running_rows+4, 0, '{}'.format(results_complete))
        stdscr.refresh()
        time.sleep(0.1)
        stdscr.clear()
...
def full_sync_threads(f_cfg,event):
    ...
    curses.noecho()
    curses.curs_set(0)
    logger.info('full_sync_threads:{}...'.format(event['tab']))
    f_cfg = get_sync_tables_thread(f_cfg)
    n_tab_total_rows = get_sync_table_total_rows_thread(f_cfg, event)
    thread_tasks = init_tasks_v2(f_cfg,event,n_tab_total_rows,stdscr)
    with concurrent.futures.ThreadPoolExecutor(max_workers=f_cfg['process_num']) as executor:
        futures = [executor.submit(full_sync_thread_task, *p) for p in thread_tasks]
        for future in concurrent.futures.as_completed(futures):
            if not future.result():
               print('future:',future.result())
    curses.endwin()

def full_sync_batch(cfg,event):
    logger.info('full_sync_batch:{}...'.format(event['tab']))
    i_counter = 0
    ...
    for o in cfg['sync_table']:
        if o != '':
           ...
           if check_tab_exists_pk(cfg,event) >0 and check_mysql_tab_exists(cfg, event) == 0:
              ...
              logger.info("clear `t_db_sync_log` for sync_table '{}'".format(event['tab']))
              start_time = time.time()
              print('Full sync table {}.{}...'.format(event['schema'], event['table']))
              full_sync_threads(copy.deepcopy(f_cfg),event)
              print('Full sync table {}.{} complete,elaspse:{}s.'.
                     format(event['schema'], event['table'], int(time.time() - start_time)))
    cfg['db_mysql'].commit()
...
def get_tables_thread(cfg,o):
    ...
    for i in list(rs):
        evt = {'schema': o.split('$')[0].split('.')[0], 'table': i[0]}
        if check_tab_exists_pk_thread(cfg,evt)>0:
           vv1 = vv1 + '{}.{}.{}${},'.format(sdb, i[0], col, ddb)
           vv2 = vv2 + '{}.{},'.format(sdb,i[0])
    return vv1[0:-1],vv2[0:-1]
...
def get_sync_tables_thread(cfg):
    v1 = ''
    v2 = ''
    for o in cfg['sync_table'].split(','):
        if o != '':
            t1, t2 = get_tables_thread(cfg, o)
            v1 = v1 + t1 + ','
            v2 = v2 + t2 + ','
    ...
    try:
        ...
        for binlogevent in stream:
            ...
            if get_seconds(apply_time) >= cfg['apply_timeout']:
               ...
               tmp,_ = get_config_from_db(cfg['sync_tag'],workdir)
               if tmp is None:
...
if __name__ == "__main__":
    ...
    if not get_task_status(tag,workdir):
       ...
       # call api get config
       cfg,f_cfg = get_config_from_db(tag,workdir)
       if cfg is None:
