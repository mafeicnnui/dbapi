        ...
        for j in range(len(rs_source)):
            ...
            if check_sqlserver_tab_exists_pk(config, tab) == 0:
                ...
                if check_mysql_tab_exists(config, get_mapping_tname(full_tab_name)) > 0:
                    ...
                    cr_desc.execute(
                        'alter table {0} add {1} int'.format(get_mapping_tname(full_tab_name), config['sync_col_name']))
                    print("Table:{0} add column {1} success!".format(get_mapping_tname(full_tab_name),
                                                                     config['sync_col_name']))
...
def sync_incr_delete(config,ftab):
    ...
    cr_dest.close()
    write_log(config,"DB:{0},Table :{1} delete {2} rows!".format(config['db_mysql_string'],get_mapping_tname(tab),len(minus)))

def sync_after_prcess(config,ftab):
    db_dest    = config['db_mysql']
    cr_dest    = db_dest.cursor()
    tab        = ftab.split(':')[0]
    v_sql_del  = "delete from {0} where sitekey not in('P00002','P00005','P00007')".format(get_mapping_tname(tab))
    v_sql_234  = "update {0} set market_id='234' where sitekey='P00002'".format(get_mapping_tname(tab))
    v_sql_164  = "update {0} set market_id='164' where sitekey='P00005'".format(get_mapping_tname(tab))
    v_sql_213  = "update {0} set market_id='213' where sitekey='P00007'".format(get_mapping_tname(tab))
    cr_dest.execute(v_sql_del)
    print(v_sql_del+' success!')
    cr_dest.execute(v_sql_234)
    print(v_sql_234 + ' success!')
    cr_dest.execute(v_sql_164)
    print(v_sql_164 + ' success!')
    cr_dest.execute(v_sql_213)
    print(v_sql_213 + ' success!')
    db_dest.commit()
    cr_dest.close()
...
def sync_sqlserver_init(config, debug):
    ...
    for i in config['sync_table'].split(","):
        ...
        if (check_mysql_tab_exists(config, get_mapping_tname(tab)) == 0 \
                or (check_mysql_tab_exists(config, get_mapping_tname(tab)) > 0 and check_mysql_tab_sync(config,
                                                                                                        get_mapping_tname(
                                                                                                            tab)) == 0)):
            # write init dict
...
def sync_sqlserver_data_pk(config, ftab, config_init):
    ...
    if not check_full_sync(config, ftab.split(':')[0]) and not config_init[ftab.split(':')[0]]:
        ...
        if ftab.split(':')[1] == '':
            ...
            print("Sync Table increment :{0} for In recent {1} {2}...".format(ftab.split(':')[0], ftab.split(':')[2],
                                                                              config['sync_time_type']))

        if ftab.split(':')[1] == '':
            print(
                'DB:{0},delete {1} table data please wait...'.format(config['db_mysql_string'], get_mapping_tname(tab)))
            cr_desc.execute('delete from {0}'.format(get_mapping_tname(tab)))
        ...
        while rs_source:
            ...
            print(
                "\r{0},Scanning table:{1},{2}/{3} rows,elapsed time:{4}s...".format(get_time(), get_mapping_tname(tab),
                                                                                    str(n_rows), str(n_tab_total_rows),
...
def sync_sqlserver_data_nopk(config, ftab, config_init):
    ...
    if check_full_sync(config, ftab.split(':')[0]) and not config_init[ftab.split(':')[0]]:
        ...
        if ftab.split(':')[1] == '':
            ...
            print('DB:{0},delete {1} table recent {2} {3} data please wait...'.format(config['db_mysql_string'], tab,
                                                                                      ftab.split(':')[2],
                                                                                      config['sync_time_type']))
...
def sync_sqlserver_data(config, config_init):
    ...
    for v in config['sync_table'].split(","):
        ...
        sync_incr_delete(config, v)
        sync_after_prcess(config, v)
