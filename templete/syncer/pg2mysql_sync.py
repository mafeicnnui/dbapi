...
def get_ds_pg(ip,port,service ,user,password):
    conn = psycopg2.connect(database=service, user=user, password=password, host=ip, port=port,cursor_factory=psycopg2.extras.DictCursor)
    return conn
    ...
    try:
        ...
        if res['code'] == 200:
            ...
            try:
                ...
                config['db_mysql_desc2']     = get_ds_mysql(db_dest_ip, db_dest_port, db_dest_service, db_dest_user, db_dest_pass)
                return config
...
def check_pg_tab_exists_pk(db,schema,tab):
   ...
   st = """select count(0) from  pg_constraint 
             where  conrelid in(select oid from  pg_class where relname='{}') 
	          and connamespace=(select oid from pg_namespace where nspname='{}') and  contype='p' """.format(tab,schema)
   cr.execute(st)
   rs=cr.fetchone()
   cr.close()
   db.commit()
...
def check_pg_tab_exists_uk(db,schema,tab):
   ...
   st = """select count(0) from  pg_constraint 
             where  conrelid in(select oid from  pg_class where relname='{}') 
	          and connamespace=(select oid from pg_namespace where nspname='{}') and  contype='u' """.format(tab,schema)
   cr.execute(st)
   rs=cr.fetchone()
   cr.close()
   db.commit()
